...
module.exports = {
	...
	customizeCombo: (req, res) => {
		...
		request({ url: sails.config.projectConfig.flowerCompanyUrl + '/getAllFlowerList', method: 'GET' }, function(
			error,
			response,
			body
		) {
			if (error) {
				console.log(error);
				return res.view('pages/errorpage', { message: 'Server down. Please try again' });
			} else {
				let flowers = JSON.parse(body);
				request(
					{ url: sails.config.projectConfig.basketCompanyUrl + '/getAllBasketList', method: 'GET' },
